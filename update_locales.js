...
const updateTranslations = () => {
  ...
  ur.dashboard.welcome = "ہیلو";
  
  en.dashboard.activeOrders = "Orders in Progress";
  ur.dashboard.activeOrders = "چل رہے آرڈر";
  
  en.dashboard.completedOrders = "Done Orders";
  ur.dashboard.completedOrders = "مکمل آرڈر";
  ...
  en.dashboard.searchPlaceholder = "Search Order Number";
  ur.dashboard.searchPlaceholder = "آرڈر نمبر ڈھونڈیں";
  
  // CUSTOMER DIRECTORY
  en.customers.title = "My Customers";
  ur.customers.title = "میرے گاہک";
  
  en.customers.subtitle = "See all your customers and their sizes.";
  ur.customers.subtitle = "اپنے تمام گاہکوں کو یہاں دیکھیں۔";
  
  en.customers.searchPlaceholder = "Search name or number";
  ur.customers.searchPlaceholder = "نام یا نمبر سے ڈھونڈیں";
  
  en.customers.addCustomer = "+ New Customer";
  ur.customers.addCustomer = "+ نیا گاہک";
  
  // NEW ORDER FORM
  en.quickOrder.customerInformation = "Step 1: Customer"; // Or "About the Customer"? The prompt says 'Step 1: Customer' for CUSTOMER INFO step label. Let's use "About the Customer" for the section. Actually I'll check how it's used. Let's just map all exact strings.
  ...
  const extractAndWalkUr = (urObj, enObj) => {
      // Create equivalent UR where possible directly matching EN or old UR
      for (let k in unupdatedEn) {
          // just apply replacement
      }
  };
  
  // Actually, we should just translate what's in 'en' using the ur map
  const updateUrFromEn = (urObj, enObj) => {
      for(let k in urObj) {
        if(typeof urObj[k] === 'string') {
           // lookup the modified english value
           // getting the matching english key path value
           // just map strings in UR, but UR might have urdu defaults!
        } else if (typeof urObj[k] === 'object' && urObj[k] !== null) {
          updateUrFromEn(urObj[k], enObj[k]);
        }
      }
  };
  
  // Simplify UR
  const walkAndReplaceUr = (obj) => {
      for (let k in obj) {
          if (typeof obj[k] === 'string') {
              for(let [key, val] of Object.entries(exactUrMap)) {
                  // The EN string was replaced by `key`, we need to find what UR should be.
                  // Wait, exactUrMap maps Simple English to Simple Urdu!
                  // We should traverse EN, and set UR corresponding key to exactUrMap[en_val] 
              }
          } else if (typeof obj[k] === 'object' && obj[k] !== null) {
              walkAndReplaceUr(obj[k]);
          }
      }
  }
