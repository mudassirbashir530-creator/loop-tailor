...
export function OrderTimeline({ currentStatus, statusHistory = {} }: OrderTimelineProps) {
  if (currentStatus === ORDER_STATUS.CANCELLED) {
    ...
    return (
      <div className="w-full bg-red-50 border border-red-100 rounded-3xl p-6 flex flex-col items-center justify-center text-center space-y-3">
        <XCircle className="h-10 w-10 text-red-500" />
        <h3 className="text-red-700 font-bold text-lg">Order Cancelled</h3>
    ...
    <div className="w-full">
          ...
          return (
            <div key={step.status} className="flex gap-4 items-start relative pb-8 last:pb-0">
              ...
              {index < timelineSteps.length - 1 && (
                <div 
                  className={cn(
                    ...
                    isCompleted ? "bg-[#2ECC71]" : "bg-gray-200"
                  )} 
                />
              )}
              ...
              <div className="relative shrink-0 z-10">
                {isCurrent && (
                  <motion.div 
                    className="absolute inset-0 rounded-full bg-[#0D3D33] opacity-20"
                ...
                <div 
                  className={cn(
                    "w-12 h-12 rounded-full flex items-center justify-center border-4 border-white shadow-sm transition-colors duration-300 relative z-10",
                    isCurrent ? "bg-[#0D3D33] text-white border-[#0D3D33]" :
                    isCompleted ? "bg-[#2ECC71] text-white border-[#2ECC71]" :
              ...
              <div className="flex-1 pt-1 ml-1">
                <h4 
                  className={cn(
                    ...
                    isCurrent ? "text-[#0D3D33] font-black" :
      ...
      <div className="hidden md:block w-full py-4">
        <div className="flex items-start justify-between relative px-6">
          ...
          {/* Active Linking Line */}
          <div 
            className="absolute top-6 left-12 h-0.5 bg-[#2ECC71] z-0 transition-all duration-500 ease-in-out"
