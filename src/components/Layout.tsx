      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 header flex items-center justify-between z-30">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {isMenuOpen && (
              <div className={cn("absolute mt-4 w-64 bg-white rounded-[20px] shadow-[0_8px_32px_rgba(34, 197, 94,0.12)] border border-[#F8FAFC] py-4 z-50", isRTL ? "left-0" : "right-0")}>
                <div className="px-6 pb-4 mb-2 mx-4 rounded-xl p-4 bg-[#F1F5F9]">
                  <div className="font-bold text-[17px] truncate text-[#22C55E]">{shop.name || t('layout.myShop')}</div>
                ...
                <Link to="/dashboard/settings" className="block px-6 py-3 text-[15px] font-medium text-[#334155] hover:text-[#22C55E]" onClick={() => setIsMenuOpen(false)}>{t('layout.settings')}</Link>
      ...
      <aside className="hidden lg:flex w-72 bg-white flex-col z-20 border-r border-[#F8FAFC]">
        <div className="h-24 flex items-center justify-between px-6 mb-4">
          <Link to="/dashboard" className="flex items-center cursor-pointer">
            <motion.div 
              ...
              className={cn("bg-[#22C55E] p-3 rounded-2xl shadow-sm", isRTL ? "ml-3" : "mr-3")}
            >
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive 
                    ? "bg-[#22C55E] shadow-[0_6px_20px_rgba(34, 197, 94,0.3)] text-white" 
                    : "text-[#334155] hover:bg-[#F5F7FA] hover:text-[#22C55E]"
                )}
              >
                <div className={cn("p-2 rounded-xl transition-all duration-300", isActive ? "bg-[#2A6B52]" : "group-hover:bg-white", isRTL ? "ml-4" : "mr-4")}>
                  <Icon className={cn("h-5 w-5", isActive ? "text-white" : "text-[#64748B] group-hover:text-[#22C55E]")} />
                </div>
                {item.name}
              </Link>
            );
        ...
        <div className="p-6 space-y-4">
          <div className="flex items-center gap-4">
            <button
              onClick={toggleLanguage}
              className="flex-1 flex items-center px-5 py-4 rounded-[14px] text-[15px] font-bold text-[#334155] hover:bg-[#F5F7FA] hover:text-[#22C55E] transition-all duration-300 group"
            >
              <div className={cn("p-2 rounded-xl group-hover:bg-white transition-all duration-300", isRTL ? "ml-4" : "mr-4")}>
                <Globe className="h-5 w-5 text-[#64748B] group-hover:text-[#22C55E] transition-colors" />
              </div>
