...
export function getGarmentType(order?: { dressType?: string; clothingType?: string } | null): string {
  if (!order) return 'Suit';
  return order.dressType || order.clothingType || 'Suit';
}
