...
export function useChat(activeCustomerId?: string) {
  ...
  const [isFallbackMode, setIsFallbackMode] = useState(() => {
    return safeGetItem('loop_tailor_chat_use_fallback') === 'true';
  });
  ...
  const getLocalChannels = useCallback((): ChatChannel[] => {
    try {
      const data = safeGetItem('loop_tailor_chats_channels');
  ...
  const saveLocalChannels = useCallback((chans: ChatChannel[]) => {
    try {
      safeSetItem('loop_tailor_chats_channels', JSON.stringify(chans));
    } catch (e) {}
  }, []);

  const getLocalMessages = useCallback((cid: string): ChatMessage[] => {
    try {
      const data = safeGetItem(`loop_tailor_chats_messages_${cid}`);
      return data ? JSON.parse(data) : [];
    } catch (e) {
      return [];
    }
  ...
  const saveLocalMessages = useCallback((cid: string, msgs: ChatMessage[]) => {
    try {
      safeSetItem(`loop_tailor_chats_messages_${cid}`, JSON.stringify(msgs));
    } catch (e) {}
  }, []);
  ...
  const enableFallbackMode = useCallback(() => {
    if (!isFallbackMode) {
      ...
      try {
        safeSetItem('loop_tailor_chat_use_fallback', 'true');
      } catch (e) {}
