...
const trustStats = [
  { label: 'Orders managed monthly', value: '85k+' },
  { label: 'Active tailoring teams', value: '1,800+' },
  { label: 'Average time saved', value: '9 hrs / week' },
  { label: 'Customer revisit uplift', value: '27%' },
];
...
export default function Landing() {
  ...
  return (
    <div className={cn('min-h-screen bg-[#FDFCF9] text-slate-900 selection:bg-brand-primary/15', isRTL ? 'font-urdu' : '')}>
      ...
      >
        <motion.div className="h-[2px] origin-left bg-brand-primary/70" style={{ scaleX: progressX }} />
      ...
      <section className="lt-section pt-10">
      ...
      <motion.section
        ...
        transition={{ duration: 0.35 }}
      >
        <div className="lt-container grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="lt-card p-7">
            <div className="flex items-center justify-between">
              <p className="lt-eyebrow">Live operations preview</p>
              <div className="rounded-full border border-brand-primary/20 bg-brand-primary/10 px-3 py-1 text-xs font-semibold text-brand-primary">
                Real-time
              </div>
            </div>
            <h3 className="lt-heading-3 mt-3">A visual control center for daily tailoring throughput.</h3>
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-3">
              {[
                { icon: Gauge, title: 'Fast dispatch', subtitle: 'Delivery lag reduced' },
                { icon: Sparkles, title: 'Cleaner flow', subtitle: 'Less context switching' },
                { icon: Star, title: 'Better service', subtitle: 'Higher repeat clients' },
              ].map((metric) => (
                <div key={metric.title} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                  <metric.icon className="h-4 w-4 text-brand-primary" />
                  <p className="mt-2 text-sm font-semibold text-slate-900">{metric.title}</p>
          ...
          <div className="lt-card p-6">
            <p className="lt-eyebrow">Customer trust loop</p>
            <h3 className="mt-3 text-xl font-semibold text-slate-900">From first visit to repeat orders, everything stays connected.</h3>
            <div className="mt-6 space-y-3">
              {[
                { step: '01', title: 'Client intake', desc: 'Capture profile, style preference, and urgency.' },
                { step: '02', title: 'Measurement record', desc: 'Save precise specs with reusable templates.' },
                { step: '03', title: 'Progress communication', desc: 'Give clear updates to reduce customer anxiety.' },
                { step: '04', title: 'Delivery + feedback', desc: 'Close orders with quality and retention insight.' },
              ].map((item) => (
                <div key={item.step} className="flex gap-3 rounded-2xl border border-slate-200 p-3">
                  <div className="h-8 w-8 rounded-lg bg-brand-primary/10 text-brand-primary text-xs font-semibold grid place-items-center">{item.step}</div>
                  <div>
      ...
      <motion.section
        className="lt-section border-y border-slate-200/70 bg-white/60"
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, amount: 0.2 }}
        variants={sectionReveal}
