...
fetch('/firebase-applet-config.json')
  .then(response => {
    ...
    if (!response.ok || !contentType || !contentType.includes('application/json')) {
      throw new Error('Config file not found or is not JSON');
    }
  ...
  .catch(err => {
    console.warn('Skipping background messaging initialization: No valid firebase config found.', err.message);
  });
